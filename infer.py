...
def save_gt_pred_vis(model, dataset, idx, out_path,
    ...
    if tgt["masks"].numel() > 0:
        gt_label = class_union_from_instances(
            tgt["masks"].float(), tgt["labels"], export_size, export_size, thr=0.5
        )
    else:
    ...
    if keep.numel() > 0:
        ...
        pr_label = class_union_from_instances(pmasks, plabels, export_size, export_size, thr=0.5)
    else:
    ...
    # side-by-side save
    canvas = Image.new("RGB", (export_size * 2, export_size))
    canvas.paste(left_overlay,  (0, 0))
    canvas.paste(right_overlay, (export_size, 0))
...
if __name__ == "__main__":
    ...
    out_path = save_gt_pred_vis(
        model=model,
        dataset=dataset,
        idx=10,                    # pick an index
        out_path="viz/gt_pred_005.png",
        device=device,
        train_size=IMG_SIZE,
